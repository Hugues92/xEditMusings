...
function ProcessMembers(e: IInterface): integer;
var
  s, c, x: string;
  t: IInterface;
  i: integer;
...
begin
  ...
  if doThisOne then begin
	...
	x := Copy(x, Length(ModName)+2, Length(x));
	// if debug then slText.Add('Processing member: '+x+' as '+IntToStr(ElementType(e)));
    
	slText.Add(Name(e)+'='+GetEditValue(e));
  end;
  
end;

function ProcessElements(e: IInterface): integer;
var
  s, c, x: string;
  t: IInterface;
  i: integer;
...
begin
  ...
  if InfoMode = 3 then
	...
	if doThisOne then begin
		...
		x := FullPathToFilename(x);
		x := Copy(x, Length(ModName)+2, Length(x));
		// if debug then slText.Add('Processing element: '+x+' as '+IntToStr(ElementType(e)));

		slText.Add('['+x+']');

		for i := 0 to ElementCount(e) - 1 do begin
			ProcessMembers(ElementByIndex(e, i));
		end;
		slText.Add('');
		for i := 0 to ElementCount(e) - 1 do begin
			ProcessElements(ElementByIndex(e, i));
		end;
	end;
...
begin
	...
	// if debug then AddMessage('Processing: '+PathName(e)+' as '+IntToStr(Ord(ElementType(e))));
	if ElementType(e) = etMainRecord then begin
		...
		if DirectoryExists(x) then begin
		  x := x+c;
		  while (Length(x)>0) and (x[Length(x)]=' ') do Delete(x, Length(x), 1);
		  if ModName = '' then ModName := FullPathToFilename(PathName(e));
		  
		  s := GetEditValue(e);
		  slText.Add('// '+FullPathToFilename(PathName(e)));
		...
		// if debug then AddMessage('Not Processed: '+PathName(e)+' as '+IntToStr(Ord(DefType(e))));
	end;
	for i := 0 to ElementCount(e) - 1 do
