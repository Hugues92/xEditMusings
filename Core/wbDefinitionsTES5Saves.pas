...
var saveFormVersion : integer = 0;

function GetSaveFormVersion(aBasePtr: Pointer; aEndPtr: Pointer; const aElement: IwbElement; var aSaveFormVersion: integer): boolean;
var
  Element : IwbElement;
  Container: IwbDataContainer;
  CT : Integer;
begin
  if saveFormVersion > 0 then begin
    result := true;
    aSaveFormVersion := saveFormVersion;
  end;
  Result := false;
  if not Assigned(aElement) then Exit;
  Element := aElement;
  ...
  if Supports(Element, IwbContainer, Container) then begin
    if SaveVersionGreaterThan11Decider(aBasePtr, aEndPtr, aElement) = 0 then
      Element := Container.ElementByPath['Save File Header\SC\Content\Form Version']
    else begin
      Element := Container.ElementByPath['Save File Header\CT'];
      if Assigned(Element) then begin
        CT := Element.NativeValue;
        case CT of
          0: Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\Content\Form Version'];
          1: Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\DataZ\Form Version'];
          2: Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\DataLZ\Form Version'];
        end;
      end;
    end;
    if Assigned(Element) then begin
      SaveFormVersion := Element.NativeValue;
      Result := true;
    end;
  end;
end;
...
begin
  Result := 0;
  if GetSaveFormVersion(aBasePtr, aEndPtr, aElement, aType) then
    if aType>aMinimum then
      Result := 1;
end;

function SaveFormVersionEqualDecider(aValue: integer; aBasePtr: Pointer; aEndPtr: Pointer; const aElement: IwbElement): Integer;
var
  aType : Integer;
begin
  Result := 0;
  if GetSaveFormVersion(aBasePtr, aEndPtr, aElement, aType) then
    if aType = aValue then
      Result := 1;
end;
...
var
  aType : Integer;
begin
  Result := SaveFormVersionEqualDecider(55,aBasePtr, aEndPtr, aElement);
end;
...
begin
  ...
  if Supports(Element, IwbContainer, Container) then begin
    Element := Container.ElementByPath['Save File Header\Save File Header\SC\Content\File Location Table\'+aName+' Count'];
    if not Assigned(Element) then Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\DataZ\Content\File Location Table\'+aName+' Count'];
    if not Assigned(Element) then Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\DataLZ\Content\File Location Table\'+aName+' Count'];
    if Assigned(Element) then begin
      Result := Element.NativeValue;
    end;
  end;
...
function xZSaveSizer(aBasePtr: Pointer; aEndPtr: Pointer; const aElement: IwbElement; var CompressedSize: Integer): Cardinal;
var
...
begin
  ...
  if Assigned(aElement) then Element := aElement.Container;
  if Assigned(Element) and Supports(Element, IwbDataContainer, Container) then begin
    if Assigned(Element) then Element := Element.Container;
    if Assigned(Element) and Supports(Element, IwbDataContainer, Container) then
      Result := SaveDataSizer(Container.DataBasePtr, Container.DataEndPtr, Element, CompressedSize);
  end;

//  if Assigned(aElement) and Supports(aElement.ResolvedValueDef, IwbStructCDef, Struct) then
...
//    ,wbArray('Remaining',  WbByteArray('Unknown', wbBytesToGroup), DumpCounter) // Lets you dump an arbitrary number of quartet, Setable from CommandLine -btd:n
  ...
  wbSaveContent := wbStruct('Content', [
    ...
    ,wbUnion('LP', SaveFormVersionGreaterThan77Decider, [wbNull, wbArray('Light plugins', wbLenString('LightPluginName', 2), -2)])
    ,wbFileLocationTable
    ,wbSaveChapters
  ]);
      ...
      ,wbStructC('Compressed',
        ...
        ,[
          ...
          ,wbUnion('Data', SaveDataDecider, [
             wbSaveContent
            ,wbStructZ ('DataZ', xZSaveSizer, nil, nil, nil, [ wbSaveContent ])
            ,wbStructLZ('DataLZ', xZSaveSizer, nil, nil, nil, [ wbSaveContent ])
           ])
         ])
     ])
    ,wbByteArray('Unused', SkipCounter) // Lets you skip an arbitrary number of byte, Setable from CommandLine -bts:n
