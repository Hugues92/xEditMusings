...
begin
  ...
  if wbToolMode in [tmDecompress] then begin
    if Supports(aElement, IwbCompressibleStruct, Struct) and (Struct.IsCompressed <> scNone) then
      Struct.IsCompressed := scNone;
  end;
...
begin
  ...
  try
    try
      ...
      tms := [tmDump, tmExport, tmDecompress];
      ...
      DoInitPath;
      if (wbToolMode in [tmDump, tmDecompress]) and (wbDataPath = '') then // Dump can be run in any directory configuration
     ...
     {$ENDIF}
      ...
      if (wbToolMode in [tmDump, tmDecompress]) and (ParamCount >= 1) and not FileExists(s) then begin
      ...
      if NeedsSyntaxInfo or (ParamCount < 1) or FindCmdLineSwitch('?') or FindCmdLineSwitch('help') then begin
        ...
        WriteLn(ErrOutput);
//        WriteLn(ErrOutput, wbAppName + 'Decompress will load the specified esp/esm files and all it''s masters and will save it to a new file with all structures decompressed.');
//        WriteLn(ErrOutput);
        WriteLn(ErrOutput, wbAppName + 'Decompress -Saves Decompress will load the specified save or coSave file and all it''s masters and will save it to a new file with all structures decompressed.');
        WriteLn(ErrOutput);
      ...
      if wbToolMode in [tmDump, tmDecompress] then begin
      ...
      if wbToolMode in [tmDump, tmDecompress] then
      ...
      ReportProgress('Finished loading record. Starting work.');
      ...
      if wbToolMode in [tmDump, tmDecompress] then begin
        if FindCmdLineSwitch('check') and not wbReportMode then
          ...
          if wbToolMode in [tmDecompress] then begin
            var NewFileName : String := ChangeFileExt(s, ExtractFileExt(_File.FileName)+'_decompress');
            FileStream := TBufferedFileStream.Create(NewFileName, fmCreate, 1024 * 1024);
            try
