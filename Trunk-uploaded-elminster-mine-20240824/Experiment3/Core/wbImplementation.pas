...
type
  ...
  TwbStruct = class(TwbValueBase, IwbCompressibleStruct)
  protected
    ...
    procedure SetIsCompressed(CompressedType : TwbStructCompression);
...
procedure TwbStruct.SetIsCompressed(CompressedType: TwbStructCompression);
begin
  DecompressIfNeeded;
  NotifyChanged(eContainer);
  case CompressedType of
    scNone: if not (scNone = IsCompressed) then begin
        szCompressedSize := -1;
        szUncompressedSize := 0;
        szCompressedType := scNone;
      end;
    scZComp: szCompressedType := scZComp;  // Nothing currently
    scLZComp: szCompressedType := scLZComp; // Nothing cuurently
  end;
end;
...
begin
  if (szCompressedSize = 0) then begin
    DoInit(False);
    ...
    if Supports(vbValueDef, IwbStructZDef, szDef)  then begin
      ...
      if szUncompressedSize <> 0 then
        szCompressedType := scZComp;
