...
begin
  ...
  if Supports(Element, IwbContainer, Container) then begin
	  ...
	  if not Assigned(Element) then Element := Container.ElementByPath['Save File Header\SC\Compressed\Content\Form Version'];
    if Assigned(Element) then begin
      aType := Element.NativeValue;
      if aType>aMinimum then
        Result := 1;
...
begin
  ...
  if Supports(Element, IwbContainer, Container) then begin
    Element := Container.ElementByPath['Save File Header\SC\Content\Form Version'];
    if not Assigned(Element) then Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\DataZ\Content\Form Version'];
    if not Assigned(Element) then Element := Container.ElementByPath['Save File Header\SC\Compressed\Data\DataLZ\Content\Form Version'];
    if Assigned(Element) then begin
      aType := Element.NativeValue;
      if aType = 55 then
        Result := 1;
...
begin
  ...
  if Assigned(Element) and Supports(Element, IwbDataContainer, Container) then
    SaveDataSizer(Container.DataBasePtr, Container.DataEndPtr, Element, CompressedSize);
...
begin
  ...
  if Supports(Element, IwbContainer, Container) then begin
    Element := Container.ElementByPath['Save File Header\CT\Compression Type'];
    if Assigned(Element) then begin
      Result := Element.NativeValue;
