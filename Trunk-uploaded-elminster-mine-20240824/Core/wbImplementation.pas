...
type
  ...
  protected
    ...
    procedure WriteToStream(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean); override;
    procedure WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean); override;
...
procedure TwbFile.WriteToStream(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
var
  MemoryStream: TMemoryStream;
begin
  MemoryStream := TMemoryStream.Create;
  try
    inherited WriteToStream(MemoryStream, aResetModified, aExpand);
...
procedure TwbFile.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
var
  SelfRef : IwbContainerElementRef;
begin
  SelfRef := Self as IwbContainerElementRef;
...
procedure TwbContainer.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
var
  i: Integer;
  SelfRef : IwbContainerElementRef;
begin
  ...
  inherited;
  for i := Low(cntElements) to High(cntElements) do
    cntElements[i].WriteToStream(aStream, aResetModified, aExpand);
...
procedure TwbMainRecord.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
var
  KAR       : IwbKeepAliveRoot;
  MS        : TMemoryStream;
  NeedReset : Boolean;
  ...
  begin
    ...
    if (esModified in eStates) or wbTestWrite or aExpand then begin
      ...
      if mrStruct.mrsFlags.IsCompressed and not aExpand then begin

        MemoryStream := TMemoryStream.Create;
        try
          inherited WriteToStreamInternal(MemoryStream, aResetModified, False);
          NeedReset := False;
        ...
        inherited WriteToStreamInternal(Stream, aResetModified, aExpand);
        NeedReset := False;
      end;
...
begin
  if not (dcfDontSave in dcFlags) then begin
    if (esModified in eStates) or (dcfBasePtrInvalid in dcFlags) or wbTestWrite or (srStruct.srsDataSize = 0) or aExpand then begin
      SelfRef := Self as IwbContainerElementRef;
      DoInit(True);

      if ((aResetModified = rmYes) or aExpand) and (dcfStorageInvalid in dcFlags) then
        UpdateStorageFromElements;
...
procedure TwbGroupRecord.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
var
  CurrentPosition   : Int64;
  NewPosition       : Int64;
  DataSize          : Cardinal;
...
procedure TwbElement.WriteToStream(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
{$IFDEF USE_CODESITE}
var
  Log: Boolean;
{$ENDIF}
begin
  ...
  try
  {$ENDIF}
    WriteToStreamInternal(aStream, aResetModified, aExpand);
  {$IFDEF USE_CODESITE}
  finally
    if Log then begin
      CodeSite.Send('aStream.Position', aStream.Position);
...
procedure TwbElement.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
begin
  Exclude(eStates, esUnsaved);
  case aResetModified of
    rmYes: begin
...
procedure TwbDataContainer.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
var
  OldPosition  : Int64;
  Size         : NativeUInt;
...
procedure TwbStringListTerminator.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
const
  NullChar : AnsiChar = #0;
begin
...
//procedure TwbChapter.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified; aExpand: Boolean);
//var
//  i         : Integer;
//  Container : IwbContainer;
//  c         : TwbStructCompressionHelper;
//begin
//    if (szCompressedType = scNone) or not Supports(Self, IwbContainer, Container) then
//      inherited
//    else begin
//      c := GetCompression;  // memorize what we read.
//      try
