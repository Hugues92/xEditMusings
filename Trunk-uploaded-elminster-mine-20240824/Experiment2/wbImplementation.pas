...
type
  ...
  protected
    ...
    procedure ExpandToStream(aStream: TStream; aResetModified: TwbResetModified); virtual;
    procedure ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified); virtual;
  ...
  protected
    ...
    procedure ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified); override;
...
procedure TwbFile.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  SelfRef : IwbContainerElementRef;
begin
  SelfRef := Self as IwbContainerElementRef;
  PrepareSave;
  inherited ExpandToStreamInternal(aStream, aResetModified);
...
procedure TwbContainer.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  i: Integer;
  SelfRef : IwbContainerElementRef;
begin
  ...
  inherited;
  for i := Low(cntElements) to High(cntElements) do
    cntElements[i].ExpandToStream(aStream, aResetModified);
...
procedure TwbMainRecord.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  KAR          : IwbKeepAliveRoot;
  MS           : TMemoryStream;
  NeedReset    : Boolean;
  ...
  begin
    ...
    mrs := mrStruct^;
    IsCompressed := mrs.mrsFlags.IsCompressed;
    try
      mrs.mrsFlags.SetCompressed(False);
      ...
      if CurrentPosition + wbSizeOfMainRecordStruct + mrStruct.mrsDataSize <> aStream.Position then
        ...
        mrs.mrsFlags.SetCompressed(IsCompressed);
      end;
  end;
...
procedure TwbSubRecord.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  CurrentPosition   : Int64;
  NewPosition       : Int64;
...
procedure TwbGroupRecord.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  CurrentPosition   : Int64;
  NewPosition       : Int64;
  DataSize          : Cardinal;
...
procedure TwbElement.ExpandToStream(aStream: TStream; aResetModified: TwbResetModified);
{$IFDEF USE_CODESITE}
var
  Log: Boolean;
{$ENDIF}
begin
  ...
  try
  {$ENDIF}
    ExpandToStreamInternal(aStream, aResetModified);
  {$IFDEF USE_CODESITE}
  finally
    if Log then begin
      CodeSite.Send('aStream.Position', aStream.Position);
...
procedure TwbElement.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
begin
...
procedure TwbStruct.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  i         : Integer;
  Container : IwbContainer;
...
begin
  if (szCompressedType = scNone) then
    ...
    try
      ...
      inherited;
      for i := 0 to Pred(Container.ElementCount) do
        Container.Elements[i].ExpandToStream(aStream, aResetModified);
    finally
...
procedure TwbDataContainer.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  OldPosition  : Int64;
  Size         : NativeUInt;
...
begin
  ...
  if (aStream.Position - OldPosition) <> ExpectedSize then
    if DoCheckSizeAfterWrite then
      Assert(False, '[TwbDataContainer.ExpandToStreamInternal] (aStream.Position - OldPosition) <> ExpectedSize');
...
procedure TwbStringListTerminator.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
const
  NullChar : AnsiChar = #0;
begin
...
procedure TwbChapter.ExpandToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  i         : Integer;
  Container : IwbContainer;
  c         : TwbStructCompressionHelper;
