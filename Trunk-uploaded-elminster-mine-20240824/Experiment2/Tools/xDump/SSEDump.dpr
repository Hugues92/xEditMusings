...
begin
  ...
  if wbToolMode in [tmDump, tmDecompress] then begin
    ...
    if DumpHidden or ((aElement.Name <> 'Unused') and (Name <> 'Unused')) then begin
      ...
      //if (wbToolMode in [tmDecompress]) and (Value <> '') then
        aElement.ExpandToStream(OutputStream, rmNo);
    end;
  end;
  ...
  try
    try
      ...
      if NeedsSyntaxInfo or (ParamCount < 1) or FindCmdLineSwitch('?') or FindCmdLineSwitch('help') then begin
        ...
        WriteLn(ErrOutput, '  or     '+wbAppName+'Decompress [options] inputfile');
        WriteLn(ErrOutput);
        ...
        WriteLn(ErrOutput);
        WriteLn(ErrOutput, wbAppName + 'Decompress will dump the plugin, decompressing all embeded compressed records.');
        WriteLn(ErrOutput, wbAppName + 'Decompress -Saves will dump the specified save or coSave file, decompressing all embeded compressed records.');
        WriteLn(ErrOutput);
      ...
      if wbToolMode in [tmExport, tmDecompress] then begin
        wbLoadBSAs := False;
        wbReportMode := False;
        wbMoreInfoForUnknown:= False;
      ...
      if wbToolMode in [tmDecompress] then begin
        if (ParamCount > 1) and not (ParamStr(Pred(ParamCount))[1] in SwitchChars) then
          t := ParamStr(Pred(ParamCount))
        else
          t := '';
        if t <> '' then try
          OutputStream := TFileStream.Create(t, fmCreate);
        except
          t := '';
        end;
        if t = '' then begin
          ReportProgress('Output file is required.');
          WriteLn(ErrOutput, 'Output file is required.');
          Exit;
        end;
        WriteContainer(_File);
        //_File.ExpandToStream(OutputStream, rmNo);
      end;
