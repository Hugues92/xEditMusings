...
var
  ...
  begin
    ...
    Uncompress := (wbToolMode in [tmDecompress]);

    if (esModified in eStates) or wbTestWrite or Uncompress then begin
      SelfRef := Self as IwbContainerElementRef;
      DoInit(True);
      ...
      if mrs.mrsFlags.IsCompressed then begin
...
procedure TwbStruct.WriteToStreamInternal(aStream: TStream; aResetModified: TwbResetModified);
var
  i  : Integer;
  sc : TwbStructCompression;
begin
  if not (wbToolMode in [tmDecompress]) or (IsCompressed = scNone) then begin
    inherited;
    Exit;
  end;
  sc := scNone;
  aStream.Write(sc, SizeOf(TwbStructCompression));
  case szCompressedType of
    scZComp: begin
      aStream.Write(szUncompressedSize, SizeOf(Cardinal));
      aStream.Write(szUncompressedSize, SizeOf(Cardinal));
    end;
    scLZComp: begin
      aStream.Write(szUncompressedSize, SizeOf(Cardinal));
  ...
  for i := Low(Elements)+3 to High(Elements) do
    Elements[i].WriteToStreamInternal(aStream, aResetModified );
