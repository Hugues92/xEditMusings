...
begin
  ...
  if aContainer.Skipped then begin
    if ((not wbReportMode) or DumpCheckReport) and not (wbToolMode in [tmDecompress]) then WriteLn(aIndent, '<contents skipped>');
  end else begin
  ...
  try
    try
      ...
      if NeedsSyntaxInfo or (ParamCount < 1) or FindCmdLineSwitch('?') or FindCmdLineSwitch('help') then begin
        ...
        WriteLn(ErrOutput, '  or     '+wbAppName+'Decompress [options] outputfile inputfile');
        WriteLn(ErrOutput);
      ...
      if wbToolMode in [tmDecompress] then begin
        ...
        _File.WriteToStream(OutputStream, rmNo);
      end;
